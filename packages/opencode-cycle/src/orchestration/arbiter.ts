...
${input.mode === "full" ? `A rejection by either reviewer binds. Disagreeing with one means rejecting with a repair target and your reasoning on record, not approving over it: the control plane records an approval that contradicts a live rejection, refuses it by name in the audit chain and routes the run to repair regardless, so approving changes nothing except what the record says you decided.
